...
const browserOpenPromise = puppeteer.launch({headless:false, defaultViewport:null, args:["--start-maximized"]});
...
.then(()=>{
    const btn = tab.click(".ui-btn.ui-btn-large.ui-btn-primary.auth-button.ui-btn-styled");
    return btn;
})
.then(()=>{
...
.then(()=>{
    const waitforquestionsATags = tab.waitForSelector('.js-track-click.challenge-list-item', {visible:true});
...
.then((allATags)=>{
    ...
    for(let i=0;i<allATags.length;i++){
        const linkPromise = tab.evaluate(elem=>{ return elem.getAttribute("href") }, allATags[i]);
...
.then((allLinksData)=>{
    ...
    const allQuestionsSolvePromise = solveQuestions(newAllLinks, 0);
...
function waitAndClick(selector){
    return new Promise((resolve, reject)=>{
        tab.waitForSelector(selector, {visible:true})
            .then((data)=>{
...
function getTag(){
    return new Promise((resolve, reject)=>{
        const elementPresentPromise = tab.waitForSelector('div[data-attr2="Editorial"]', {visible:true});
        elementPresentPromise.then(()=>{
            const isTagPresentPromise = tab.evaluate(()=>{
                return document.querySelector('div[data-attr2="Editorial"] .ui-icon-lock');
            })
        ...
        .then((isTagPresent)=>{
            ...
            const getTagname = tab.evaluate(()=>{
                ...
                return document.querySelector('div[data-attr2="Editorial"] .ui-icon-lock').className==="ui-icon-lock";
            })
            return getTagname;
        })
        .then((isPresent)=>{
...
function getPermission(){
    return new Promise((resolve, reject)=>{
        const waitforLockedEditorial = tab.waitForSelector('.ui-btn.ui-btn-normal.ui-btn-primary.ui-btn-styled', {visible:true});
    ...
    return new Promise((resolve, reject)=>{
        ...
        .then((allH3)=>{
            ...
            for(let i=0;i<allH3.length;i++){
                const textPromise = tab.evaluate((elem)=>{return elem.textContent;}, allH3[i])
        ...
        .then((codeAndIndex)=>{
            ...
            for(let i=0;i<codeAndIndex[0].length;i++){
                const codesPromise = tab.evaluate((elem)=>{return elem.textContent;},codeAndIndex[0][i])
...
function pasteCode(code){
    return new Promise((resolve, reject)=>{
        ...
        .then(()=>{
            const clickOnEditorPromise = tab.click('.monaco-editor.no-user-select.vs');
...
function solve(link){
    return new Promise((resolve, reject)=>{
        ...
        .then(()=>{
            const whetheriTagisPresent = getTag();
            return whetheriTagisPresent;
        })
        .then((isiTagPresent)=>{ 
            let getCodePromise; 
            if(isiTagPresent===false){
                getCodePromise = getCode();
            }else{
                getCodePromise = getPermission();
            }
            return getCodePromise;
        })
        .then((code)=>{
        ...
        .then(()=>{
            const pressSubmitPromise = tab.click('.pull-right.btn.btn-primary.hr-monaco-submit');
