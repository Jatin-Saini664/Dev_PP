...
(async function(){
    ...
    let link = await tab.evaluate(elem => {return elem.getAttribute("href");}, createChallengeBtn);
...
async function addAllModerator(browser, tab){
    ...
    for(let i=0;i<allchallenge.length;i++){
        let challengeLink = await tab.evaluate(elem => {return elem.getAttribute("href")}, allchallenge[i]);
    ...
    let isDisabled = await tab.evaluate( function(elem){ return elem.classList.contains("disabled"); } , secondLastLi );
    if(isDisabled){
        return;
    }else{
...
async function addChallenge(completeLink, browser, challenge){
    ...
    const allCodeMirror = await newPage.$$('.CodeMirror.cm-s-default.CodeMirror-wrap');
