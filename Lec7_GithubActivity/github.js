...
function parseTopic(error, response, data) {
  ...
  let allProjectArticleTags = ch(
    ".border.rounded.color-shadow-small.color-bg-secondary.my-4"
  );
  // {  "0":{} , "1":{} , "2" : {}  }
  // console.log(topicName);
  for (let i = 0; i < 5; i++) {
...
function workOnSingleProject(projectArticleTag, topicName) {
  // project name
  // issue link
  let projectName = cheerio(projectArticleTag)
    .find("a.text-bold")
    .text()
    .trim();
